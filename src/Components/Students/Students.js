...
import InfoTypeSection from '../../Shared/Components/InfoTypeSection/StudentInfoType/InfoTypeSection';
...
const Students = props => {
    ...
    useEffect(() => {
        ...
        const fetchData = async () => {
            try {
                const responseData = await sendRequest(process.env.REACT_APP_BACKEND_URL + 'students/' + archiveMode);
                dispatch(actions.loadedData(responseData.students));
            } catch (err) {
    ...
    const addButtonHandler = () => {
        dispatch(actions.infoTypeChange('students'));
        dispatch(actions.toggleBackdrop(true));
    ...
    const searchInputHandler = e => {
        ...
        if (e.target.value.length === 0) {
            ...
            searchedData = adminData.filter(data =>
                ...
                || data.surname.toLowerCase().includes(e.target.value.toLowerCase()) ||
    ...
    return (
        <React.Fragment>
            <div className='info-type-panel'>
                ...
                <Borderline classBorderline='info-type-panel__borderline' />
                <DataAdminModal />
            </div>
            {loading ? <Spinner classSpinner='spinner-centered' /> : (!errorModalActive ?
